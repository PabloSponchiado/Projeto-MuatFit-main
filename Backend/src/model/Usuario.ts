...
export class Usuario {
    ...
    static async cadastroUsuario(usuario: Usuario): Promise<string | null> {
        try {
            ...
            const valores = [usuario.nome, usuario.email.toLowerCase(), usuario.senha, 'ADMIN', 'Minha Academia'];
            ...
            const uuid = String(resultado.rows[0].id_usuario);
    ...
    static async atualizarImagemPerfil(uuid: string, nomeArquivo: string): Promise<void> {
        ...
        const query = `UPDATE usuario SET imagem_perfil = $1 WHERE id_usuario = $2`;
