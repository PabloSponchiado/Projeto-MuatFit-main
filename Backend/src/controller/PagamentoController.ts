...
const PagamentoController = {
  ...
  async create(req: Request, res: Response) {
    ...
    const result = await pool.query(
      ...
      [alunoId, alunoNome, valor, dataVencimento, dataPagamento, status, mes, ano, observacao]
    )
  ...
  async confirm(req: Request, res: Response) {
    const { id } = req.params
    await pool.query("UPDATE pagamentos SET status = 'PAGO', data_pagamento = CURRENT_DATE WHERE id = $1", [id])
    return res.json({ id, confirmado: true })
  },
